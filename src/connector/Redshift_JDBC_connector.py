...
class RedshiftJDBC(Connector):
    def create_connection(self):
        list_driver = IOUtils.get_src_path(os.getcwd())
        try:
            cnn = jaydebeapi.connect("com.amazon.redshift.jdbc42.Driver",
                                     f"""jdbc:redshift://{self.server}:{self.port}/{self.database}""",
                                     [self.user, self.pw],
                                     list_driver)
            return cnn
        except Exception:
